...
parser.add_argument("--pretrain-model", default="", type=str, help="type like: ckpt,0")
...
if args.pretrain_model:
    ...
    model_name = "{}_{}.pth".format(args.pretrain_model.split(",")[0], args.pretrain_model.split(",")[1])
...
def adjust_lr(tmp_optimizer, epoch):
	...
	lr = args.lr * (0.1 ** (epoch // 50)); print("learning rate is {}".format(lr))
...
def test(epoch):
    ...
    # if True:
        ...
        #     os.mkdir('checkpoint')
        if save_best:
            torch.save(state, os.path.join(args.ckpt_dir, 'ckpt.pth'))
        else:
